...
to calculate-distance
 ...
 ; set real-distance laps *
  set real-distance distancexy 0 0
...
to go
  ...
  cop-vision
  ask turtles with [speed > 0] [calculate-distance]
  tick
...
to move-forward ; turtle procedure
 ...
 ; if y-distance <= 0 [die]
  if blocking-car != nobody [
    ...
    slow-down-car
    ;if xcor - distance min-one-of blocking-cars  [distance myself] < .3 [set number-of-cars number-of-cars - 1 die]
  ]


  ask turtles with [patience <= 0] [become-road-rager]
  forward speed
end


to become-road-rager
  set color red
  set speed  0.6 + random-float .15 ;60 + 0-10mph :60-75
  set top-speed .9
  set road-rager? true
end
...
to choose-new-lane ; turtle procedure
  ...
  if not empty? other-lanes [
    ...
    set target-lane abs one-of closest-lanes
 ...
 ; display cop vision
  ask turtles with [cop? = true ]
   ...
   ; (1 + speed) 180 with [ y-distance <= 3 ]
    let blocking-car min-one-of blocking-cars [ distance myself ]
    if blocking-car != nobody and [cop?] of blocking-car = false and [speed] of blocking-car > 0   [
        ask blocking-car [set speed .55]]
  ]
...
215
498
initial-number-of-road-rage-drivers
initial-number-of-road-rage-drivers
0
number-of-cars - number-of-cops
5.0
1
1
NIL
...
837
505
Distance Traveled (selected car)
[laps] of selected-car
5
1
11
...
455
Road Ragers
count turtles with [road-rager? = true]
17
1
11
...
455
Nromal Drivers
count turtles with [cop? = false and road-rager? = false]
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Control Exp:1" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <metric>min [speed] of turtles</metric>
    <metric>max [speed] of turtles</metric>
    <metric>mean[speed] of turtles</metric>
    <metric>min [patience] of turtles</metric>
    <metric>max [patience] of turtles</metric>
    <metric>mean[patience] of turtles</metric>
    <enumeratedValueSet variable="distance-to-travel">
      <value value="50000"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-number-of-road-rage-drivers">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-of-cops">
      <value value="2"/>
    </enumeratedValueSet>
